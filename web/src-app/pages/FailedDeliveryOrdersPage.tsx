...
const FailedDeliveryOrdersPage = () => {
  ...
  const fetchOrdersForToken = async (access_token: string, updateAfterISO: string, updateBeforeISO: string) => {
    try {
      ...
      // Status 'shipped_back' matches mobile FailedDeliveryOrders.tsx
      let requestUrl = `${BASE_URL}/get-daraz-delivered-order-details?access_token=${access_token}&update_after=${encodeURIComponent(updateAfterISO)}&update_before=${encodeURIComponent(updateBeforeISO)}&status=shipped_back`;
      ...
      console.error("Error fetching failed delivery orders:", error);
      return [];
    }
  };
  ...
  return (
    <div style={{
      ...
      <Header title={AppStrings.FailedDeliveredOrders} goBack={() => navigate(-1)} />
      ...
      <div style={{ marginTop: '24px' }}>
        {loading && darazOrders.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '40px', color: AppColors.textSecondary }}>
            Loading failed delivery orders...
          </div>
